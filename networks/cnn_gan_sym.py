	...
	class Discriminator(nn.Module):
		def __init__(self):
			...
			self.main = nn.Sequential(
				...
				nn.Conv2d(nc, ndf, (3, 3), (2, 2), 0, bias=False),
				nn.BatchNorm2d(ndf),
				nn.LeakyReLU(0.2, inplace=True),
				# state size. (ndf) x 32 x 32
				nn.Conv2d(ndf, ndf * 2, (3, 3), (2, 2), 0, bias=False),
				nn.BatchNorm2d(ndf * 2),
				nn.LeakyReLU(0.2, inplace=True),
				# state size. (ndf*2) x 16 x 16
				nn.Conv2d(ndf * 2, ndf * 4, (3, 3), (2, 2), 0, bias=False),
				nn.BatchNorm2d(ndf * 4),
				nn.LeakyReLU(0.2, inplace=True),
				# state size. (ndf*4) x 8 x 8
				nn.Conv2d(ndf * 4, 1, (4, 1), (2, 1), 0, bias=False),
				nn.Sigmoid()
			)
