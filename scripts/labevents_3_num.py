...
with open('out/labevents_filtered_num.csv') as fin:
	reader = csv.reader(fin, delimiter=',', quotechar='"')
	next(reader, None)
	for line in reader:
...
with open('out/labevents_num_codes.tsv', 'w+') as fout:
	for c in codes_list:
...
with open('out/labevents_num_steps.tsv', 'w+') as fout:
...
np.save('out/dims/3-labevents_num-1', matrix)